...
s3_tokenizer_config <- function(n_mels = 128, n_audio_state = 1280,
    ...
    list(n_mels = n_mels, n_audio_ctx = 1500, n_audio_state = n_audio_state,
...
precompute_freqs_cis <- function(dim, end, theta = 10000.0) {
    ...
    freqs <- 1.0 / (theta ^ (torch::torch_arange(start = 0, end = dim - 1,
                step = 2, dtype = torch::torch_float32())[1:(dim %/% 2)] / dim))
...
s3_log_mel_spectrogram <- function(audio, mel_filters, window, n_fft = 400,
    ...
    # Compute STFT
    stft <- torch::torch_stft(audio, n_fft = n_fft, hop_length = S3_HOP,
...
fsmn_multi_head_attention <- torch::nn_module(
    ...
    self$fsmn_block <- torch::nn_conv1d(n_state, n_state, kernel_size,
                                        stride = 1L, padding = 0L,
                                        groups = n_state, bias = FALSE)
...
s3_residual_attention_block <- torch::nn_module(
    ...
    n_mlp <- n_state * 4L
    self$mlp <- torch::nn_sequential(torch::nn_linear(n_state, n_mlp),
                                     torch::nn_gelu(),
...
s3_tokenizer <- torch::nn_module(
    ...
    mel_fb <- create_mel_filterbank(sr = S3_SR, n_fft = 400L,
                                    n_mels = config$n_mels, fmin = 0,
                                    fmax = S3_SR / 2)
    ...
    # Hann window
    self$window <- torch::nn_buffer(torch::torch_hann_window(400L))
    ...
    list(tokens = result$tokens, lens = result$lens)
}
)
...
load_s3tokenizer_weights <- function(model, state_dict, prefix = "tokenizer.") {
    ...
    torch::with_no_grad({
        ...
        for (i in seq_along(model$encoder$blocks)) {
            ...
            # Attention layer
            copy_if_exists(block$attn$query$weight,
                           paste0(block_prefix, "attn.query.weight"))
