...
snake_activation <- torch::nn_module(
    ...
    x + (1.0 / (alpha + self$no_div_by_zero)) * torch::torch_pow(torch::torch_sin(x * alpha),
        2)
}
)
...
conv_rnn_f0_predictor <- torch::nn_module(
    ...
    self$condnet <- torch::nn_sequential(
        torch::nn_conv1d(in_channels, cond_channels, kernel_size = 3,
                         padding = 1),
        torch::nn_elu(),
...
sine_gen <- torch::nn_module(
    ...
    torch::with_no_grad({
        ...
        F_mat <- torch::torch_zeros(c(batch_size, self$harmonic_num + 1,
                                      seq_len),
                                    device = device)
...
hift_generator <- torch::nn_module(
    ...
    self$m_source <- source_module_hn_nsf(sample_rate = sampling_rate,
        upsample_scale = total_upsample, harmonic_num = nb_harmonics,
        sine_amp = nsf_alpha, add_noise_std = nsf_sigma,
