...
.get_mel_banks <- function(num_bins, window_length_padded, sample_freq,
    ...
    stopifnot(low_freq >= 0, low_freq < nyquist, high_freq > 0,
...
.povey_window <- function(window_size) {
    torch::torch_hann_window(window_size, periodic = FALSE,
                             dtype = torch::torch_float32())$pow(0.85)
}
...
.kaldi_get_window <- function(waveform, padded_window_size, window_size,
    ...
    if (preemphasis_coefficient != 0) {
        ...
        offset_strided_input <- torch::nnf_pad(strided_input$unsqueeze(1),
            c(1, 0), mode = "replicate")$squeeze(1) # (m, window_size + 1)
...
kaldi_fbank <- function(audio, num_mel_bins = 80L, sample_rate = 16000) {
    ...
    strided_input <- .kaldi_get_window(waveform, padded_window_size,
