...
espnet_rel_positional_encoding <- torch::nn_module(
    ...
    self$pe <- torch::nn_buffer(self$create_pe(max_len, d_model))
},
    ...
    position <- torch::torch_arange(0, max_len - 1,
                                    dtype = torch::torch_float32())$unsqueeze(2)
...
linear_no_subsampling <- torch::nn_module(
    ...
    initialize = function(input_dim = 512, output_dim = 512,
                          dropout_rate = 0.1)
    {
    self$out <- torch::nn_sequential(torch::nn_linear(input_dim, output_dim),
...
pre_lookahead_layer <- torch::nn_module(
    ...
    self$conv1 <- torch::nn_conv1d(channels, channels,
...
rel_position_attention <- torch::nn_module(
    ...
    self$pos_bias_u <- torch::nn_parameter(torch::torch_zeros(c(n_head,
                self$d_k)))
...
upsample_1d <- torch::nn_module(
    ...
    # Interpolate 2x
    x <- torch::nnf_interpolate(x, scale_factor = as.double(self$stride),
                                mode = "nearest")
...
upsample_conformer_encoder_full <- torch::nn_module(
    ...
    # Pre-lookahead layer
    self$pre_lookahead_layer <- pre_lookahead_layer(output_size,
        pre_lookahead_len)
...
load_conformer_encoder_weights <- function(model, state_dict,
    ...
    # Pre-lookahead layer
    copy_if_exists(model$pre_lookahead_layer$conv1$weight,
